...
static int stenv_eval_drop_target(int player, int hand_index, Card* hand_card, int deck_index)
{
    CardSet own_backup;
    CardSet deck_backup;
...
int ai_stenv_drop(int player)
{
    ...
    for (i = 0; i < g.own[player].num; i++) {
        ...
        if (target_count <= 0) {
            ...
            gain = stenv_eval_drop_target(player, i, hand_card, deck_index) - current_score;
        ...
        for (int ti = 0; ti < target_count; ti++) {
            int gain = stenv_eval_drop_target(player, i, hand_card, targets[ti]) - current_score;
            if (!found || gain > best_gain || (gain == best_gain && i < best_hand_index)) {
                best_gain = gain;
                best_hand_index = i;
                found = ON;
    ...
    if (g.own[player].num > 0 && g.own[player].cards[0]) {
        return stenv_find_hand_index(player, g.own[player].cards[0]);
    }
    return 0;
}
